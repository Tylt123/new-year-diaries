(function (window, document, $) {
    ...
    $.tmEPOAdmin = {
        ...
        "can_take_logic": ["color", "range", "radiobuttons", "checkboxes", "selectbox", "textfield", "textarea", "variations"],
        ...
        add_events: function () {
            ...
            tcuploader.bind("Error", function (uploader, error) {
                if (error && error.message) {
                    $(".tm_info").removeClass("tm_color_success").addClass("tm_color_error").html("\nError #" + error.code + ": " + error.message);
                }
                $(".tm_progress_bar").removeClass("tm_notice").addClass("tm_error");
                $(".flasho").removeClass("tm_color_notice tm_color_success").addClass("tm_color_error");
            });
            ...
            $(document).on("change.cpf", ".product-default-value-search", function () {
                ...
                $.post(TMEPOGLOBALADMINJS.ajax_url, data, function (response) {
                    ...
                    if (mode.val() === "categories" && productId !== $this.attr("data-current-selected")) {
                        catArray = $(".product-categories-selector").find(":selected").toArray().map(function (x) {
                            return parseInt($(x).val());
                        });
                        valid = false;
                        $.each($this.data("categoryIds"), function (i, id) {
                            if ($.inArray(parseInt(id), catArray) !== -1) {
                                valid = true;
                                return false;
                            }
                        });
            ...
            $(document).on("change.cpf", ".product-categories-selector", function () {
                ...
                var element = $(this);
                var catArray = element.find(":selected").toArray().map(function (x) {
                    return parseInt($(x).val());
                });
                var productDefaultValue = $(".product-default-value-search");
                var valid;
        ...
        make_resizables: function (obj) {
            ...
            obj.not(".tma-nomove").toArray().forEach(function (el) {
                ...
                el.resizable({
                    ...
                    start: function (event, ui) {
                        ...
                        startLimit = parseFloat($.tmEPOAdmin.builder_size[ui.originalElement.attr("class").split(" ").filter(function (x) {
                    ...
                    stop: function (event, ui) {
                        ...
                        currentSize = bitem.attr("class").split(" ").filter(function (x) {
                            return x.indexOf("w") === 0;
                        })[0];
        ...
        sections_type_onChange: function (sectionIndex) {
            ...
            if (style === "slider" && !builder_wrapper.hasClass("tm-slider-wizard")) {
                ...
                TCBUILDER[sectionIndex].section.sections_slides.default = builder_wrapper.find(".bitem_wrapper").map(function (i, e) {
                    return $(e).children(".bitem").not(".pl2").length;
                }).get().join(",");
            ...
            if (product_type.length) {
                if (product_type.val() === "variable" || product_type.val() === "variable-subscription") {
                    ...
                    if (!variation_element.length) {
                        ...
                        if ($.tmEPOAdmin.getFieldValue($.tmEPOAdmin.variationSection, "variations_disabled") === "1") {
                            is_forced = true;
                            variation_element = $(".builder_layout .element-variations");
                            $.tmEPOAdmin.var_is("tm-style-variation-forced", true);
                        }
        ...
        tcSerializeJsOptionsArray: function (options) {
            ...
            options = [].concat.apply(
                ...
                [].concat.apply(
                    ...
                    options.map(function (x) {
                        return Object.keys(x.section).map(function (key) {
            ...
            options = options.map(function (y) {
                if (y) {
        ...
        tcSerializeJsOptionsObject: function (options) {
            ...
            $.each(a, function () {
                if (o[this.name] !== undefined) {
                    ...
                    if (this.value !== null && this.value !== undefined && this.value.push) {
                        o[this.name] = [this.value];
                    } else {
                        o[this.name] = this.value || "";
                    }
                ...
                if (section !== undefined) {
                    ...
                    Object.keys(fields).forEach(function (ii) {
                        ...
                        if (name !== undefined) {
                            ...
                            field_type = (elementType === "variations" ? "variation" : ($.inArray(elementType, [
                                "radiobuttons",
                                "checkboxes",
                                "selectbox"]) !== -1) ? "multiple" : "text");
        ...
        logic_append: function (el, options, type, sectionIndex, fieldIndex) {
            ...
            logic.empty();
            h = "";
            h = "<div class=\"tc-row tm-logic-rule\">" + "<div class=\"tc-cell tc-col-4 tm-logic-element\">" + "</div>" + "<div class=\"tc-cell tc-col-2 tm-logic-operator\">" + "</div>" + "<div class=\"tc-cell tc-col-4 tm-logic-value\">" + "</div>" + "<div class=\"tc-cell tc-col-2 tm-logic-func\">" + "<button type=\"button\" class=\"tmicon tcfa tcfa-plus add cpf-add-rule\"></button>" + " <button type=\"button\" class=\"tmicon tcfa tcfa-times delete cpf-delete-rule\"></button>" + "</div>" + "</div>";
            rule = $(h);
        ...
        element_logic_reindex: function () {
            ...
            Object.keys(TCBUILDER).forEach(function (sectionIndex) {
                ...
                fieldsNoDisabled = TCBUILDER[sectionIndex].fields.filter(function (x) {
                    var type = $.tmEPOAdmin.getFieldValue(x, "element_type");
                    return $.tmEPOAdmin.getFieldValue(x, "enabled", type) === "1" || $.tmEPOAdmin.getFieldValue(x, "enabled", type) === undefined;
                });
                ...
                Object.keys(TCBUILDER[sectionIndex].fields).forEach(function (fieldIndex) {
                    ...
                    uniqid = $.tmEPOAdmin.getFieldValue(field, "uniqid", true);
                    field_index = fieldsNoDisabled.findIndex(function (x) {
                        var type = $.tmEPOAdmin.getFieldValue(x, "element_type");
                        return uniqid === $.tmEPOAdmin.getFieldValue(x, "uniqid", type) && ($.tmEPOAdmin.getFieldValue(x, "enabled", type) === "1" || $.tmEPOAdmin.getFieldValue(x, "enabled", type) === undefined);
                    });
                    ...
                    $.each(element_rules.rules, function (i, rule) {
                        ...
                        if (!(l.end.section === "delete" && copy.element === "delete")) {
                            copy_rules[i] = $.tmEPOAdmin.validate_rule(copy, sectionIndex, fieldIndex, field_index);
                        }
        ...
        validate_rule: function (rule, sectionIndex, fieldIndex, field_index) {
            ...
            if (!globalVariationObject || !rule || typeof rule !== "object" || rule.element === undefined || rule.operator === undefined || rule.section === undefined || (rule.value === undefined && !(rule.operator === "isempty" || rule.operator === "isnotempty") )) {
        ...
        builder_plus_onClick: function () {
            ...
            var currentSize = bitem.attr("class").split(" ").filter(function (x) {
                return x.indexOf("w") === 0;
            })[0];
            var keys = Object.keys($.tmEPOAdmin.builder_size);
        ...
        builder_section_plus_onClick: function () {
            ...
            var currentSize = section.attr("class").split(" ").filter(function (x) {
                return x.indexOf("w") === 0;
            })[0];
            var keys = Object.keys($.tmEPOAdmin.builder_size);
            var currentIndex = keys.findIndex(function (x) {
        ...
        builder_section_item_onClick: function () {
            ...
            if (TCBUILDER[sectionIndex].section.tma_variations_wrap === true) {
                ...
                _clone.find(".tma-tab-logic,.tma-tab-css,.tma-tab-woocommerce").hide();
            }
                ...
                if (builder[i].id === "multiple") {
                    Object.keys(builder[i].multiple).forEach(function (ii) {
                        ...
                        if (options_wrap.eq(ii).length === 0) {
                            options_wrap.eq(0).tcClone().appendTo(panels_wrap)
                            .find(":input").not("button")
                            .val("")
                            .removeAttr("checked").prop("checked", false)
                            .find(".tm_option_enabled").prop("checked", true).val("1").trigger("changechoice");
                        }
                        ...
                        Object.keys(builder[i].multiple[ii]).forEach(function (iii) {
                            ...
                            element = content.find(".options_wrap").eq(ii).find("[name^='" + name + "']");
                            element.attr("data-builder", JSON.stringify(builder[i].multiple[ii][iii]));
                            element.attr("name", builder[i].multiple[ii][iii].tags.name)
                            .attr("data-field-id", builder[i].multiple[ii][iii].id)
                            .removeAttr("id")
                            .val(value)
                            .trigger("change");

                            if (builder[i].multiple[ii][iii].checked !== undefined) {
                                if (builder[i].multiple[ii][iii].id === "multiple_checkboxes_options_default_value" || builder[i].multiple[ii][iii].id === "multiple_radiobuttons_options_default_value" || builder[i].multiple[ii][iii].id === "multiple_selectbox_options_default_value") {
                                    element.val(ii);
                                } else {
                    ...
                    element
                    .attr("data-builder", JSON.stringify(builder[i]))
                    .attr("data-field-id", builder[i].id);
        ...
        builder_item_onClick: function () {
            ...
            elementType = $.tmEPOAdmin.getFieldValue(TCBUILDER[sectionIndex].fields[fieldIndex], "element_type");
            _clone = $(_template).filter(".bitem.element-" + elementType).tcClone(true);
            original_enabled = $.tmEPOAdmin.getFieldValue(TCBUILDER[sectionIndex].fields[fieldIndex], "enabled", elementType);
            original_enabled = (elementType === "variations" || original_enabled === "1") ? "1" : "0";
            if (TCBUILDER[sectionIndex].section.tma_variations_wrap === true) {
                _clone.find(".tma-tab-logic,.tma-tab-css,.tma-tab-woocommerce").remove();
            }
            ...
            $("#temp_for_floatbox_insert").find(".inside").addClass("tm-hidden").after($("<div class=\"floatbox-loading\">" + TMEPOGLOBALADMINJS.i18n_loading + "</div>"));
            setTimeout(function () {
                var rules;
        ...
        getFieldObject: function (_clone) {
            ...
            _clone.find(":input").not("button").not(".panels_wrap :input").toArray().forEach(function (el) {
                ...
                if (name !== undefined && name.indexOf("tm_meta[") === 0) {
                    ...
                    if (el.attr("data-builder")) {
                        ...
                        if (builder.fill === "product" || builder.fill === "category" || el.is(".product-products-selector") || el.is(".product-categories-selector")) {
                            tempObject.options = el.children("option:selected").toArray().map(function (option) {
            ...
            if (panelsWrap.length > 0) {
                ...
                _clone.find(".panels_wrap .options_wrap").each(function (i, wrap) {
                    multipleObjectWrap = [];
                    $(wrap).find(":input").not("button").each(function (ii, el) {
                        el = $(el);
                        name = el.attr("name");
        ...
        builder_clone_element: function (element, wrapper_selector, append_or_prepend) {
            ...
            if (_clone) {
                ...
                is_slider = TCBUILDER[sectionIndex].section.is_slider;
                if (is_slider) {
                    TCBUILDER[sectionIndex].section.sections_slides.default = wrapper_selector.find(".bitem_wrapper").map(function (i, e) {
                        return $(e).children(".bitem").not(".pl2").length;
                    }).get().join(",");
                }
        ...
        builder_reorder_multiple: function () {
            ...
            var obj;
            var outputArray = [].concat.apply([], TCBUILDER.map(function (x) {
                            ...
                            if (multiple !== undefined && typeof(multiple) === "object") {
                                ...
                                multiple.forEach(function (multipleObject, multipleIndex) {
                                    multipleObject.forEach(function (multipleField, multipleFieldIndex) {
                                        TCBUILDER[sectionIndex].fields[fieldIndex][foundIndex].multiple[multipleIndex][multipleFieldIndex].tags.name = $.tmEPOAdmin.nameChange(TCBUILDER[sectionIndex].fields[fieldIndex][foundIndex].multiple[multipleIndex][multipleFieldIndex].tags.name, counter);
                                    });
                                });
                            }
                            counter = counter + 1;
        ...
        create_products_search: function (_clone) {
            ...
            _clone.find(":input.wc-product-search").filter(":not(.enhanced)").each(function () {
                var select2_args = {
                    allowClear: true,
        ...
        create_enhanced_dropdown: function (_clone) {
            _clone.find(":input.enhanced-dropdown").filter(":not(.enhanced)").each(function () {
                var select2_args = {
                    allowClear: true,
                    placeholder: $(this).data("placeholder"),
                    dropdownCssClass: "tc-dropdown"
        ...
        create_normal_dropdown: function (_clone) {
            _clone.find("select").filter(":not(.enhanced, .enhanced-dropdown, .wc-category-search, .wc-product-search)").not(".builder-logic-div select, .panels_wrap select, .options_wrap select").each(function () {
                var select2_args = {
                    allowClear: false,
        ...
        paginationOnClick: function (pager, page, perpage, options_wrap) {
            ...
            options_wrap.filter(function (index) {
        ...
        variations_display_as: function (e) {
            ...
            $this.find(".tm_option_image").not(".tm_option_imagep").each(function (i) {
        ...
        tm_upload: function () {
            ...
            $this.find(".tm_option_image").not(".tm_option_imagec, .tm_option_imagep, .tm_option_imagel").each(function (i) {
        ...
        populatePriceVariables: function (obj) {
            var ids = deepCopyArray(TCBUILDER.map(function (x, sectionIndex) {
            ...
            if (ids.length) {
                ...
                list = $("<div class=\"tc-clearfix tm-epo-switch-wrapper\"><div class=\"formula-field-mode-selector\"><input checked=\"checked\" name=\"formulamode\" class=\"formula-field-mode\" id=\"formulafieldmode0\" value=\"price\" type=\"radio\" ><label for=\"formulafieldmode0\"><span class=\"tc-radio-text\">" + TMEPOGLOBALADMINJS.i18n_formula_field_price + "</span></label><input name=\"formulamode\" class=\"formula-field-mode\" id=\"formulafieldmode1\" value=\"value\" type=\"radio\" ><label for=\"formulafieldmode1\"><span class=\"tc-radio-text\">" + TMEPOGLOBALADMINJS.i18n_formula_field_value + "</span></label><input name=\"formulamode\" class=\"formula-field-mode\" id=\"formulafieldmode2\"  value=\"quantity\" type=\"radio\" ><label for=\"formulafieldmode2\"><span class=\"tc-radio-text\">" + TMEPOGLOBALADMINJS.i18n_formula_field_quantity + "</span></label><input name=\"formulamode\" class=\"formula-field-mode\" id=\"formulafieldmode3\" value=\"count\" type=\"radio\" ><label for=\"formulafieldmode3\"><span class=\"tc-radio-text\">" + TMEPOGLOBALADMINJS.i18n_formula_field_count + "</span></label></div></div>");
                obj.append(list);
    ...
    $(document).ready(function () {
        ...
        TMEPOOPTIONSJS = window.TMEPOOPTIONSJS;
